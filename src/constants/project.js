import tiendaOnline from "..//assets/img/tienda-online.png";
import inmohouse from "..//assets/img/inmohouse.png";
import meals from "..//assets/img/meals.png";
import industry from "..//assets/img/industry.png";
import movie from "..//assets/img/movie.png";
import tooltyp from "..//assets//img/tooltyp.png";


export const projects = [
  {
    title: "Inmohouse",
    description:
      "Front-end de portal inmobiliario. He  Utilizando React para la interzad de usuario, Material UI para dar estilo a los componentes y React-Router para una navegación fluida y optimizada.",
    image: inmohouse,
    tags: ["JavaScript", "React", "Material-UI, React-Router"],
    source: "",
    liveVersion: "",
    id: 0,
    enlace: "https://642eeaf8e63d6400bc514fee--sensational-tiramisu-da16eb.netlify.app/",
    codigo: "https://github.com/milagros-lab/InmoHouse"
  },
  {
    title: "appMovie",
    description:
      "App creada con React, redux, mui, firebase para la autenticación de usuario. Se trata de una plataforma digital para reproducir peliculas. Se conecta a la API The Movie Database (TMDB) para generarnos un listado de películas.",
    image: movie,
    tags: ["Html", "React", "Material UI", "Firebase"],
    source: "",
    liveVersion: "",
    id: 1,
    enlace : "https://appmovie-lab.netlify.app/",
    codigo: "https://github.com/milagros-lab/appMovie"
  },
  {
    title: "Tienda online",
    description:
      "Aplicación web interactiva realizada utilizando tecnologías modernas como React para la interfaz de usuario, Firebase para la gestión de datos y la autenticación, y Material-UI (MUI) para un diseño atractivo y altamente funcional.",
    image: tiendaOnline,
    tags: ["React", "JavaScript", "Firebase", "MUI"],
    source: "",
    liveVersion: "",
    id: 2,
    enlace : "https://easychooping.netlify.app/",
    codigo: "https://github.com/milagros-lab/tienda-online"
  },
  {
    title: "Mi menú",
    description:
      "Esta aplicación se conecta a la API externa TheMealDB para obtener diferentes tipos de recetas de comida, se trata de un sitio web totalmente responsive que se adapta a diferentes dispositivos.",
    image: meals,
    tags: ["React", "JavaScript", "Axios"],
    source: "",
    liveVersion: "",
    id: 3,
    enlace : "https://meals-lab.netlify.app/",
    codigo: "https://github.com/milagros-lab/proyectos-replit"
  },
  {
    title: "Web industry",
    description:
      "El proyecto tiene como objetivo desarrollar un sitio web receptivo y estéticamente agradable utilizando HTML para la estructura básica, Bootstrap para el diseño y la funcionalidad, y SCSS para mejorar la organización y reutilización del código CSS.",
    image: industry,
    tags: ["Html", "Saas", "Bootstrap"],
    source: "",
    liveVersion: "",
    id: 4,
    enlace : "https://zesty-chebakia-c53ed2.netlify.app/",
    codigo: "https://github.com/milagros-lab/industry"
  },
  {
    title: "Tooltyp",
    description:
      "Maquetacion de web con Html, Css y Bootstrap, totalmente responsible para tamanos movil, tablet y ordenador.",
    image: tooltyp,
    tags: ["Html", "Css", "Bootstrap"],
    source: "",
    liveVersion: "",
    id: 5,
    enlace : "https://lnkd.in/d-VzRn3S",
    codigo: "https://github.com/milagros-lab/tooltyp-prueba"
  },
 
];
